...
import type { SkillModel } from "@/skill-model";
  ...
  const getPortfolioAssignment = (tech: SkillModel[], title: string, abstract: string, date: string | number | Date, thumbnail: string, techstackLogo: string, websiteUrl: string, gitRepoUrl: string, logoShape?: "sqaure" | "circle") => {
        ...
        export const SKILLS : SkillModel[]= [ getSkill("angular", angular),  getSkill("react", react),  getSkill("next", next), getSkill("vue", vue), getSkill("java", java), getSkill("python", python), getSkill("mongodb", mongo), getSkill("postgresql", postgres), getSkill("mysql", mysql), getSkill("UIUX", uiux)];
