...
export const useCartStore = defineStore('cart', () => {
  ...
  const cartItems: ComputedRef<Map<number, number>> = computed(() => {
    return countProductInCart(cart.value)
  })
  const cartMap: ComputedRef<Map<ProductModel, number>> = computed(() => {
    return transformToMap(cart.value)
  })
